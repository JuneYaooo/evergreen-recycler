...
下面是一次真实运行（合成样本夹具，基准日 2026-09-19，v0.1.0）：67 行数据体检、
...
| 能力 | 边界 |
| --- | --- |
| 赢家检测：互动量对比**平台内中位数 ≥3x 且 ≥P75**，附倍数与分位数可复核 | 只和账号自己的历史比，不是流量承诺；播放/赞/评的权重可调，口径见[字段文档](references/排期规则与字段.md) |
...
V1 评测（2026-09-19，v0.1.0，全部使用合成夹具、固定基准日 2026-09-19）：
**9/9 用例通过，0 硬失败，32/32 原子检查通过，16/16 单元测试通过**。覆盖赢家检测
（排期是否恰好落在设计的高互动帖上）、排期约束复算、稀疏平台降级、ledger 防重复、
全输家账号的诚实空结果、冷启动建议、确定性（两次运行 sha256 一致）、缺字段容忍与
变体红线九类行为。

<p align="center">
  <img src="./docs/assets/evaluation-summary.svg" alt="评测摘要：9/9 通过，0 硬失败，含缺陷修复迭代与已知限制" width="100%">
</p>
